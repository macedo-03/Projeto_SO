...
void worker_process(int worker_number, int from_dispatcher_pipe[2]){
    ...
    while (1) {
        ...
        if (message_to_process.type == 0) {//mensagem do user
            ...
            if (strcmp(main_cmd, "ADD_ALERT") == 0) {
                ...
                if (*count_alerts < MAX_ALERTS) {
                    for (i = 0; i < *count_alerts; ++i) {
                        if (strcmp(alert_list[i].alert_id, alert_id) == 0) { //if alert_id already exists reject it
#ifdef DEBUG
                ...
                sem_wait(sem_alert_list_writer);
                for (i = 0; i < *count_alerts; ++i) {
                    if (strcmp(alert_list[i].alert_id, alert_id) == 0) { //if alert_id exists, delete it
#ifdef DEBUG
            ...
            } else if (strcmp(main_cmd, "RESET") == 0) {//clean every stats/keys in the data base
...
#endif
                if (strcmp(sensor_list[i], sensor_id) == 0) { //if sensor does already exists keep processing message
#ifdef DEBUG
            ...
            if (new_sensor == 1 && *count_sensors == MAX_SENSORS) { //if it's a new sensor and the maximum number of sensors as been reached, reject the message
#ifdef DEBUG
...
#endif
                if (strcmp(data_base[i].key, key) == 0) { //if the key does already exist -> update key
#ifdef DEBUG
            ...
            if (new_key == 1 && validated) {
                if (*count_key_data < MAX_KEYS) {//if it's a new sensor and the maxiumum number of keys hasn't been reached -> create key
#ifdef DEBUG
            ...
            if (new_sensor == 1 && validated) { //if it's a new sensor and the maxiumum number of keys hasn't been reached -> create sensor
#ifdef DEBUG
...
#endif
                *count_sensors+=1;
            } else if (!validated) { //if it isn't validated -> discardthe message
...
void alerts_watcher_process(){
    ...
    while (1) {
        ...
        for (j = 0; j < *count_key_data; ++j) {
            ...
            if (keys_bitmap[j] == 1) { //this key has been modified
...
//        printf("\n%s\n", sensor_info);
        ...
        if(sem_trywait(&internal_queue_empty_count)==-1){ //menos um slot vazio
            if(errno == EAGAIN){    //if semaphore had value 0, it means the queue is full -> discard sensor message
